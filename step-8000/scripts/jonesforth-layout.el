...
(defun jonesforth-layout--sync-enabled-p ()
  "Return non-nil when trace sync is enabled."
  (let ((trace-buffer (or (get-file-buffer "trace.tree.org")
                          (get-buffer "trace.tree.org"))))
    (and trace-buffer
         (buffer-live-p trace-buffer)
         (with-current-buffer trace-buffer
           jonesforth-layout--sync-enabled))))

(easy-menu-define jonesforth-layout-menu global-map
  "Jonesforth layout menu."
  '("Jonesforth"
    ["Show Help (F1)" jonesforth-layout-show-help t]
    ["Toggle Symbol Highlight (F6)" jonesforth-layout-toggle-symbol-highlight t]
    ["Trace Sync Enabled (F8)" jonesforth-layout-toggle-sync
     :style toggle :selected (jonesforth-layout--sync-enabled-p)]
    ["Reload Layout" jonesforth-layout-reload t]))
